...
@section('admin')
    ...
    const partyType = document.querySelector('select[name="party_type"]');
    const payable = document.querySelector('input[name="opening_payable"]');
    const receivable = document.querySelector('input[name="opening_receivable"]');

    const toggleFields = () => {
        const val = partyType.value;
        payable.disabled = val === 'customer';
        receivable.disabled = val === 'supplier';
        if (val === 'both') {
            payable.disabled = receivable.disabled = false;
        }
        payable.value = payable.disabled ? '' : payable.value;
        receivable.value = receivable.disabled ? '' : receivable.value;
    };

    const handleInput = (e) => {
        if (partyType.value === 'both') {
            if (e.target === payable && payable.value) {
                receivable.disabled = true;
                receivable.value = '';
            } else if (e.target === receivable && receivable.value) {
                payable.disabled = true;
                payable.value = '';
            } else if (!payable.value && !receivable.value) {
                payable.disabled = receivable.disabled = false;
            }
        }
    };
    ...
    toggleFields();
    partyType.addEventListener('change', toggleFields);
    payable.addEventListener('input', handleInput);
    receivable.addEventListener('input', handleInput);
});
        // error remove
