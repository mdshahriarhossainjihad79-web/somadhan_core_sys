...
(function ($) {
  ...
  $(function () {
    ...
    const baseUrl = window.location.origin; // Base URL (http://127.0.0.1:8000)
    const currentPath = window.location.pathname; // Path (/sale/pharmacy)

  if (currentPath === '/sale/pharmacy' ) {
  const toggler = $('.sidebar-header .sidebar-toggler');
  toggler.toggleClass('active not-active');
